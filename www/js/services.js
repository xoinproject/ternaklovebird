angular.module("ternaklovebird.services", [])
// TODO: --|---- directive
...
.directive("runAppEmail", function(){
	return {
			...
			function runApp(event)
			{
				var EmailAddr = $attrs.email || "info@bstrdProject.com";
...
.directive("runAppFacebook", function(){
	return {
			...
			function runApp(event)
			{
				var textLink = window.encodeURIComponent($attrs.link) || "http://bstrdProject.com/";
				if (ionic.Platform.isIOS()){
...
.directive("runOpenURL", function(){
	...
	return {
			...
			function runApp(event)
			{
				var $href = $attrs.href || "http://bstrdProject.com/";
